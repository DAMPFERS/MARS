    ...
    def __init__(self, title, station_idx):
        ...
        blocks = [
            ("ЭНЕРГЕТИКА", [
                ...
                ("Генерация", "0", "МВт", 1),
                ("Накопитель", "0", "МВт", 2),
                ("Энергоэффективность", "0", "%", 13)
            ], 0, 0),
            ("СВЯЗЬ", [
                ("Скорость", "0", "Мбит/с", 3),
                ("Задержка", "0", "мс", 4),
                ("SNR", "0", "dB", 5), 
                ("Время сеанса связи", "00:00:00", "", 12)
            ], 0, 1),
            ("МАТЕРИАЛЫ", [
                ("Семена", "0/5", "", 6),
...
if __name__ == "__main__":
    ...
    if font_id != -1:  # Если шрифт загрузился успешно
        ...
        app.setFont(QFont(family, 12 * scale_factor))  # Применяем шрифт размером 10
    else:
        app.setFont(QFont("Consolas", 12 * scale_factor))  # Шрифт по умолчанию
